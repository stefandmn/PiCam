...
python $_APP_HOME/picam.py -c "init server and start service on #1 and enable property CameraStreaming on #1 and set property MotionRecordingFormat=video on #1" -i "0.0.0.0" -p 9079 -v